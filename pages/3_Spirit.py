...
# ---------------- CONFIG ----------------
st.set_page_config(page_title="Spirit Rover", layout="wide")
st.title("🔧 Spirit Rover - Mars Photo Explorer")
st.caption("Browse real images taken by NASA's Spirit rover on the surface of Mars.")
...
# ✅ Correct camera list for Spirit
camera = st.sidebar.selectbox("Camera", ["ALL", "FHAZ", "RHAZ", "NAVCAM", "PANCAM", "MINITES"])

# Initialize page number in session state
if "spirit_page" not in st.session_state:
    st.session_state.spirit_page = 1
...
with st.sidebar:
    ...
    with col1:
        if st.button("⬅️ Previous") and st.session_state.spirit_page > 1:
            st.session_state.spirit_page -= 1
    with col2:
        if st.button("Next ➡️"):
            st.session_state.spirit_page += 1
    st.markdown(f"**Current Page:** {st.session_state.spirit_page}")
...
if query_type == "Martian Sol":
    # ✅ Use a good default for Sol (Spirit is inactive now)
...
photos = get_photos(
    ...
    camera=camera,
    page=st.session_state.spirit_page
)

st.subheader(f"Results ({len(photos)} photos found on page {st.session_state.spirit_page})")
