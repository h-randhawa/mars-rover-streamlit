...
# ---------------- CONFIG ----------------
st.set_page_config(page_title="Opportunity Rover", layout="wide")
st.title("🛠️ Opportunity Rover - Mars Photo Explorer")
st.caption("Browse real images taken by NASA's Opportunity rover on the surface of Mars.")
...
photos = get_photos(
    ...
    camera=camera,
    page=st.session_state.opportunity_page
)

st.subheader(f"Results ({len(photos)} photos found on page {st.session_state.opportunity_page})")
